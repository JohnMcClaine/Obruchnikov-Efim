...
public class Calculator4 {
    ...
    public static String calc(String inputStr) throws IOException {
        ...
        if(operation == 0){
            throw new IOException("ОШИБКА! Данные не являются математической операцией ");
        }

        if(Ar > 1){
            throw new IOException("ОШИБКА! Недопустимо использование более одной операции");
        }
        ...
        if ((Objects.equals(afterSplit[0], "1") || Objects.equals(afterSplit[0], "2")
            ...
            if (result < 1) {
                throw new IOException("ОШИБКА! В римской системе счисления нет отрицательных цифр");
            } else {
                ...
                throw new IOException("ОШИБКА! Разные системы счисления");
            ...
            throw new IOException("ОШИБКА! Один из операндов не соответствует диапазону от 1 до 10");
        }
